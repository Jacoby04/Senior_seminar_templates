...
Newly allocated objects in JVM languages are stored in a contiguous memory location called
the \emph{heap}~\cite{oracle:heap}. The heap represents the memory available to a program and is a virtual layer of memory that corresponds to physical memory.
...
as the \emph{stack}, which is separate from the heap. Objects are never actually
...
Consider an example with an object that uses 2 megabytes (Mb) of memory, 
...
multi-threading. One example, relevant to GC, is that modifications to an 
...
All but one of the algorithms we examine are parallel. A \emph{concurrent} garbage
...
among multiple collectors for comparative purposes. As an example, one way to
