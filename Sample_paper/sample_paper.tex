...
C4's intended environment is large server environments with multiple gigabyte heaps.
...
is to-pages. From-pages contain garbage
and are marked as such during the tracing phase.

To support concurrent relocation, pages being relocated are protected by the LVB. 
...
\end{itemize}
In all cases, the object is not used by a mutator until it is on a to-page.
Additionally, LVB corrects the
old reference loaded by the mutator to prevent further barrier triggers on the same
reference.
