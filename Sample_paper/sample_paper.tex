...
In GC, the overarching pattern common to all algorithms is finding which object are garbage
and reclaiming the memory from those objects~\cite{wiki:gc}. This can be broken down into two major steps.
...
traversing references from global, root objects. Any objects that can be reached by chaining references
