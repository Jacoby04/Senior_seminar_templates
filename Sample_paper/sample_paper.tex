...
is to pages. From pages are any with garbage
...
the physical memory backing it can be put to use immediately. The virtual memory for the
from page will still be in use until all references have been remapped, but this allows
for efficient recycling of physical memory.

To support concurrent relocation, pages that are being relocated are protected. 
The LVB will trigger when encountering a reference pointing to an object
that is protected.
