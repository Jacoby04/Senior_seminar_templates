...
\todo[inline]{Note: above was a first attempt. I wasn't happy with it, so I am trying
to consolidate it more and really make use of what a data structures student should know.}

In object-oriented programming languages, the allocation and deallocation
of space in memory for objects can be explicit or implicit. If done explicitly,
a language is said to be \emph{unmanaged}, otherwise it is \emph{managed} 
~\cite{msdn:managedVsUnmanaged}. Examples of unmanaged languages include 
C and C++. Some managed languages are C\#, Java, and Python.
Use of implicit memory management is beneficial to programmers as it allows
using the robustness of an object-oriented language without worrying about
...
Memory for a program is not an unlimited resource. As such, managed languages
...
save space for new objects that will be created. In managed languages, the implicit
deallocation of garbage is performed by a \emph{garbage collector}.
A garbage collector is a sophisticated algorithm designed to detect
and remove dead objects for us. We will focus on specific parts of the 
...
\todo[inline]{Cite Jeff's paper and that one site I am using.}
