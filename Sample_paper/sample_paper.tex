...
counter. Threads cannot relocate objects should the number of
HPs pinning the objects be greater than zero.
...
% This is redundant with the process below.
Once a mutator is done accessing an
object, the mutator unpins it. If blamed, the mutator will attempt to perform the copy itself. If 
...
along to the impeding mutators. This sequence
occurs repeatedly until a thread tries to copy the object when it is not pinned. 
...
\end{enumerate}
\end{enumerate}
From this, we see how mutators will continually bounce blame around until
no threads have the object pinned any longer. At this time, the last thread
