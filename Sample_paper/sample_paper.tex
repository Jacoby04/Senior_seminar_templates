...
will be marked NIT since their referrer sets do not meet requirement 2 for being stable.
This means mutators are using the objects and the GC process does not want to interfere.
...
memory as the corresponding from-space. As mentioned, 
...
Mirrored-to-space is used to get the compactor to believe its work is done.
It already holds the same objects in physical memory as from-space.
Also, the compactor considers this space to be to-space. Thus, if references
to objects are changed to point to mirrored-to-space rather than from-space,
the objects appear to be relocated without physically moving. This technique
used to terminate compaction is known as zero-copy transplantation.

%Refer to figure here once it is added in a new paragraph or the paragraph right above.

To have NIT objects be moved from the perspective of the compactor, zero-copy transplantation
is performed on all of them. The LVB-style barrier gets a second purpose here. As mutators
try to access references to NIT objects before they are updated to mirrored-to-space, 
the barrier will cause them to correct the references before using the object. Also like
...
for this reason, Collie is referred to as an \emph{aborting} compactor. It will
